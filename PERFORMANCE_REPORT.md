...
**문서 갱신일**: 2026-09-16 (정적 분석 항목만 재확인 — 실측 벤치마크는 아래 참고)  
...
> ⚠️ **재측정 필요**: 아래 Cold Start/gfxinfo/메모리/배터리 실측 수치는 모두 **2026-06-15 기준**입니다.
> 이 문서 갱신 시점(2026-09-16)에는 세션에 연결된 실기기/에뮬레이터가 없어(`adb devices` 결과 없음)
> 재측정하지 못했습니다. 2026-06-15 이후 KOFIC 박스오피스 연동(홈 화면 신규 섹션, `a5524bf`/`02c28c7`)과
> 관련 접근성 수정(`940843c`)이 추가돼 홈 화면 렌더링 부하가 늘었을 가능성이 있으므로, 기기 연결 후
> `adb shell dumpsys gfxinfo`로 홈 화면 재측정을 권장합니다. 아래 "정적 렌더링/메모리 분석" 표는
> 코드 재확인으로 갱신했습니다.
...
| `onSizeChanged` 캐시 | ✅ 5곳 (2026-09-16 재확인, 이전 13곳) | 사전 계산 패턴 적용 |
| `setHasFixedSize(true)` | ✅ 8개 RecyclerView (2026-09-16 재확인, 이전 10개) | |
| LinearLayout 포함 레이아웃 | ⚠️ 31개 파일 (2026-09-16 재확인, 이전 30개) | Overdraw 잠재적 위험 — 체감 영향은 낮음 |
...
| 항목 | 상태 | 비고 |
|---|---|---|
| 어댑터 `onViewRecycled` / `dispose()` | ✅ 6개 파일 (2026-09-16 재확인, 이전 15개) | Coil 이미지 정리 |
| Fragment `_binding = null` | ✅ 9개 Fragment | ViewBinding 누수 방지 |
| `ViewSizeResolver` | ✅ 6개 파일 (2026-09-16 재확인, 이전 9개) | 실제 표시 크기로 다운샘플링 |
...
*문서 갱신(정적 분석 재확인): 2026-09-16 — Baseline Profile(`:baselineprofile` 모듈, benchmark 1.5.0) 및
R8 full mode(`android.enableR8.fullMode=true`) 설정 그대로 유효함을 확인. Coil 캐시 설정(메모리 25%,
디스크 50MB) 변경 없음. 실측 벤치마크(Cold Start/gfxinfo/메모리/배터리)는 재측정 안 됨 — 상단 경고 참조.*
