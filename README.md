...
| 실험 내용 | 도구 | 결과 |
|---|---|---|
| 시청 통계 기능 전체 설계 및 구현 | Claude Code | UseCase 48개, Repository 10개 인터페이스 |
| PR 코드 리뷰 자동화 | GitHub Actions + Claude API | PR 생성 시 자동 리뷰 코멘트 게시 |
| 테스트 커버리지 개선 | Claude Code + JaCoCo | **74.8%** 달성 |
| 라이브러리 보안 취약점 분석 | Exa MCP | 실시간 웹 검색으로 실제 버그 발견 |
| think / ultra think 프롬프트 비교 | Claude Code | ultra think: 4에이전트 병렬, **30개+ 이슈** 자동 발견 |
...
| Test | JUnit 4, MockK 1.14.9, Turbine 1.2.1 (397개 유닛 + 23개 Espresso) |
