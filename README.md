...
| Home | Search | Detail | Favorite | Settings |
|:----:|:------:|:------:|:--------:|:--------:|
| <img src="screenshots/home.png" width="180"/> | <img src="screenshots/search.png" width="180"/> | <img src="screenshots/detail.png" width="180"/> | <img src="screenshots/favorite.png" width="180"/> | <img src="screenshots/settings.png" width="180"/> |
...
| Test | JUnit 4, MockK 1.14.9, Turbine 1.2.1 (61개 테스트) |
...
│   └── util/              # 상수 (PAGE_SIZE, PREFETCH_DISTANCE, 언어 코드)
├── domain/                # 도메인 레이어 (순수 Kotlin)
│   ├── model/             # Movie, MovieDetail, Cast, Genre, ThemeMode
│   ├── repository/        # Repository 인터페이스
│   └── usecase/           # UseCase 24개
├── presentation/          # 프레젠테이션 레이어
│   ├── adapter/           # RecyclerView 어댑터 7개 + MovieGridViewHolder (공유 ViewHolder)
│   ├── common/            # CircularRatingView (커스텀 뷰, 접근성 지원)
...
└── MovieFinderApp.kt      # Application (@HiltAndroidApp, Coil/테마/Certificate Pinning 설정)
```
...
- **고급 필터**: 연도 / 장르 다중 선택 / 정렬 (인기순·평점순·개봉일순·수익순) ChipGroup
...
- 상세/출연진/비슷한 영화/예고편/등급 **5개 API 병렬 호출** (`coroutineScope` + `async`)
...
- **FAB 즐겨찾기/워치리스트 토글**: `@Transaction` 원자적 처리 + bounce 애니메이션 + 실패 시 Channel → Snackbar 피드백
...
- **테마 설정**: 라이트/다크/시스템 전환 (DataStore 저장)
- **캐시 삭제**: Coil 이미지 메모리 캐시 클리어
...
FragmentNavigatorExtras(posterView to "poster_$movieId")
  → DetailFragment: postponeEnterTransition(500ms timeout) + doOnPreDraw { startPostponedEnterTransition() }
  → TransitionSet: ChangeBounds + ChangeTransform + ChangeImageTransform
```

## Testing

총 **61개** 유닛 테스트를 작성하여 핵심 비즈니스 로직을 검증합니다.
...
| `DetailViewModelTest` | 11 | 초기 상태, ErrorType 에러, 부분 실패(credits/similar/trailer/certification), 즐겨찾기/워치리스트 토글, Snackbar, 재시도, 중복 호출 방지 |
...
- **Coil 이미지 캐시**: 메모리 25% + 디스크 5% (네트워크 요청 최소화) + `onViewRecycled()` dispose
...
- **Room 인덱스**: `CachedMovieEntity(category)`, `FavoriteMovieEntity(addedAt)`, `RecentSearchEntity(timestamp)`, `WatchHistoryEntity(watchedAt)`, `WatchlistEntity(addedAt)`에 인덱스 추가
