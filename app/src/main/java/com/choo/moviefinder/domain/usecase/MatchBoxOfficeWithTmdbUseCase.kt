...
// KOFIC 박스오피스 목록(일별/주간 공통)의 각 영화를 TMDB 검색 결과와 제목 기준으로 매칭한다.
...
@Reusable
class MatchBoxOfficeWithTmdbUseCase @Inject constructor(
    private val movieQueryRepository: MovieQueryRepository
) {
    suspend operator fun invoke(boxOfficeList: List<BoxOffice>): List<BoxOfficeMovie> = coroutineScope {
